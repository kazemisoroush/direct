...
// type in frontend/lib/config.ts. Cognito fields are added when auth lands (M1).
...
func (f frontendHosting) deploy(stack awscdk.Stack, apiURL *string) {
	config := webConfig{APIURL: apiURL}
...
// URL is the https origin the browser loads and the value added to the API CORS list.
