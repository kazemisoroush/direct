...
Order direct from restaurants and skip the delivery-app tax. Go on Lambda behind
API Gateway, DynamoDB for data, Next.js on Vercel for the storefront. Built as an
Inside-the-Box steel thread, one real restaurant end to end first.
...
```md
**🟠 Major · abstraction · [`backend/internal/order/service.go:42`](https://github.com/kazemisoroush/direct/blob/<sha>/backend/internal/order/service.go#L42)**
The service calls the Stripe SDK directly from the order-placement path.

**Why.** An external integration in business logic cannot be swapped or faked in a test.
**Fix.** Put the payment call behind a dedicated boundary the service depends on.
```
